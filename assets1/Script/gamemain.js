...
var gamemain = cc.Class({
    ...
    */
    ...
    /*
        ...
        思路: 自己为最左边没有 直接返回false 左边的第一个和自己不相同则直接返回false step 如果小于给定的寻找次数 则直接返回
    */
    checkmaskleft:function(id,step){
        if(this.getAllmask()[id].x == 0 || this.getAllmask()[id - 1].num != this.getAllmask()[id].num || this.getAllmask()[id-1].step <= step){
            return false;
        }
        if(this.getAllmask()[id-1].step==999){
            this.g_mask_samecnt++;
        }
        this.getAllmask()[id - 1].step = step;
        this.getAllmask()[id - 1].from = id;
        return true;
    },
    ...
    /*
        ...
        思路:  如果为最右边 则停止寻找 如果当前显示的数字不同 则停止寻找 如果step小于给定的Step则也停止寻找
    */
    checkmaskright:function(id,step){
        if(this.getAllmask()[id].x == 4 || this.getAllmask()[id + 1].num != this.getAllmask()[id].num || this.getAllmask()[id + 1].step <= step){
            return false;
        }
        if(this.getAllmask()[id + 1].step == 999){
            this.g_mask_samecnt ++;
    ...
    /*
        ...
        思路:  如果为最底边 则停止寻找 如果当前显示的数字不同 则停止寻找 如果step小于给定的Step则也停止寻找
    */
    checkmaskdown:function(id,step){
        if(this.getAllmask()[id].y == 0 || this.getAllmask()[id - 5].num != this.getAllmask()[id].num || this.getAllmask()[id - 5].step <= step){
           return false;
        }
        if(this.getAllmask()[id - 5].step == 999){
            this.g_mask_samecnt ++;
        }
        this.getAllmask()[id - 5].step = step;
        this.getAllmask()[id - 5].from = id;
        return true;
    },
    ...
    /*
        ...
        思路: 如果为最顶部 则停止寻找 如果当前显示的数字不同 则停止寻找 如果step小于给定的Step则也停止寻找
    */
    checkmaskup:function(id,step){
        if(this.getAllmask()[id].y == 4 || this.getAllmask()[id + 5].num != this.getAllmask()[id].num || this.getAllmask()[id + 5].step <= step){
            return false;
        }
        if(this.getAllmask()[id + 5].step == 999){
            this.g_mask_samecnt ++;
